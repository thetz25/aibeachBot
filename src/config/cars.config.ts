...
export interface CarModel {
    id: string;
    name: string;
    price: number; // Base price for calculation
    dpPercent: number; // Default DP percent (e.g., 0.20)
...
export const getCarByName = (name: string): CarModel | undefined => {
    return CAR_MODELS.find(
        car => car.name.toLowerCase().includes(name.toLowerCase())
    );
};
