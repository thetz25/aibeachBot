                ...
                for (const webhook_event of entry.messaging) {
                    ...
                    if (webhook_event.message && webhook_event.message.text) {
                        ...
                        try {
                            ...
                            const historyPromise = (0, db_service_1.getHistory)(senderId);
                            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Supabase timeout')), 3000));
                            history = await Promise.race([historyPromise, timeoutPromise]);
                        ...
                        let response;
                        try {
                            const openaiPromise = (0, openai_service_1.generateAIResponse)(receivedText, aiHistory);
                            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('OpenAI timeout')), 10000));
                            ...
                            await (0, messenger_service_1.sendMessage)(senderId, "Sorry, I'm having trouble processing your message. Please try again in a moment.");
